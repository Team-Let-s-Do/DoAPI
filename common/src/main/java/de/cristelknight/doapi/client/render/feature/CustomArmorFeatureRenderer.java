...
public class CustomArmorFeatureRenderer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
		...
		if(hatItem != null) {
			if(MODELS.isEmpty()) {
				...
				for(DoApiAPI api : apis){
					api.registerArmor(MODELS, this.modelLoader);
				}
			}
	...
	public CustomArmorModelItem getHatItem(T entity, EquipmentSlot slot)
	{
		ItemStack headSlot = entity.getItemBySlot(slot);
		if(headSlot.getItem() instanceof CustomArmorModelItem hat && !headSlot.isEmpty())
			return hat;
		return null;
	}
	...
	public void render(PoseStack matrices, MultiBufferSource vertexConsumers, int light, T entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
		...
		for(EquipmentSlot slot : slots){
			...
			ItemStack headSlot = entity.getItemBySlot(slot);
			if(headModel != null && headSlot.getItem() instanceof CustomArmorModelItem armorItem){
	...
	protected ResourceLocation getTexture(T entity, EquipmentSlot slot) {
		CustomArmorModelItem customItem = getHatItem(entity, slot);
